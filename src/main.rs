...
pub fn see_difference<A: AsRef<Path>, B: AsRef<Path>>(a_base: A, b_base: B) -> Result<(), Error> {
  ...
  // println!("{:?}", a_walker.next());
  // println!("{:?}", b_walker.next());
  for (a, b) in (&mut a_walker).zip(&mut b_walker) {
    let a = a?;
    let b = b?;
...
// fn walk_dir_and_get_files() -> walkdir::IntoIter {
//   WalkDir::new("tests/easy").into_iter().filter_map(|e| {
//     if let Ok(a) = e {
...
fn walk_dir_and_get_files() -> impl IntoIterator<Item = walkdir::DirEntry> {
  WalkDir::new("tests/easy")
    .into_iter()
...
fn main() {
  println!("{:?}", see_difference("tests/easy/bad/", "tests/easy/bad/"));
  // for entry in WalkDir::new("tests/easy").into_iter().filter_map(|e| {
  //   if let Ok(a) = e {
  //     if !a.file_type().is_dir() {
  //       return Some(a);
  //     }
